...
- Copy `.env.example` to a secure environment-specific configuration; never commit `.env`.
- Use PostgreSQL by setting `DB_NAME`, `DB_USER`, `DB_PASSWORD`, and `DB_HOST`.
- Run `python manage.py migrate` and `python manage.py setup_scheduled_tasks` once per deployment.
- Run a persistent Django-Q worker with `python manage.py qcluster` so order expiry and cleanup jobs execute.
- Terminate TLS at a trusted reverse proxy and enable `TRUST_PROXY_HEADERS` only when it overwrites forwarded headers.
- Validate production settings with `python manage.py check --deploy`.
- Run backend tests from `backend/` with `../.venv/bin/python manage.py test`.
- Run frontend checks with `npm run lint`, `npm test`, and `npm run build`.
