...
def validate_captcha(captcha_response, remote_ip=None):
    """
    ...
    verify_url = getattr(settings, 'CAPTCHA_VERIFY_URL', '')
    if not secret or not verify_url:
        logger.error('CAPTCHA is required but provider settings are missing')
        return False, 'سرویس CAPTCHA در دسترس نیست.'
    ...
    try:
        ...
        logger.exception('CAPTCHA provider verification failed')
        return False, 'اعتبارسنجی CAPTCHA انجام نشد؛ دوباره تلاش کنید.'

    if not result.get('success'):
        return False, 'CAPTCHA نامعتبر است.'
    return True, None
