...
class CreateOrderSerializer(serializers.Serializer):
    ...
    guest_email = serializers.EmailField(
        ...
        allow_blank=True,
        error_messages={
            'invalid': 'فرمت ایمیل صحیح نیست؛ لطفاً یک ایمیل معتبر مانند example@mail.com وارد کنید.'
        }
    )
    guest_phone = serializers.CharField(required=False, allow_blank=True, max_length=20)
    full_name = serializers.CharField(required=False, allow_blank=True, max_length=255)
    phone = serializers.CharField(required=False, allow_blank=True, max_length=20)
    address_line1 = serializers.CharField(required=False, allow_blank=True, max_length=500)
    address_line2 = serializers.CharField(required=False, allow_blank=True, max_length=500)
    city = serializers.CharField(required=False, allow_blank=True, max_length=100)
    state = serializers.CharField(required=False, allow_blank=True, max_length=100)
    postal_code = serializers.CharField(required=False, allow_blank=True, max_length=20)
    country = serializers.CharField(required=False, allow_blank=True, default='Iran', max_length=100)

    def validate(self, attrs):
        user = self.context['request'].user
        if user.is_authenticated:
            if not attrs.get('shipping_address_id'):
                raise serializers.ValidationError({"shipping_address_id": "آدرس ارسال الزامی است."})
        else:
            if not attrs.get('guest_email'):
                raise serializers.ValidationError({"guest_email": "ایمیل برای ثبت سفارش مهمان الزامی است."})
            required = ['full_name', 'address_line1', 'city', 'state', 'postal_code']
            missing = [f for f in required if not attrs.get(f)]
            if missing:
                raise serializers.ValidationError({"error": "تکمیل آدرس ارسال برای سفارش مهمان الزامی است."})
            if not re.fullmatch(r'\d{10}', attrs.get('postal_code', '')):
                raise serializers.ValidationError({"postal_code": "کد پستی باید دقیقاً ۱۰ رقم باشد."})
        return attrs
    ...
    def validate_coupon_code(self, value):
        ...
        user = self.context['request'].user
        valid, msg = coupon.is_valid(user=user if user.is_authenticated else None)
        if not valid:
