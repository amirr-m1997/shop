...
/** Persian clock time (HH:mm) */
export function formatTime(dateInput) {
  if (!dateInput) return '';
  return toJalali(dateInput, { format: 'timeOnly' });
}

/** Current/provided Jalali year with Persian digits */
export function formatJalaliYear(dateInput = new Date()) {
  return toJalali(dateInput, { format: 'year' });
}
