...
const CustomerClubSection = () => {
  ...
  const redeem = async (reward) => {
    ...
    if (!window.confirm(`آیا می‌خواهید «${reward.name}» را با ${Number(reward.points_required).toLocaleString('fa-IR')} امتیاز دریافت کنید؟`)) return;
        ...
        {loading ? <div className="flex items-center gap-3 rounded-2xl border border-border/50 bg-background/50 p-5 text-sm text-muted-foreground" role="status"><Loader2 className="h-5 w-5 animate-spin text-amber-500" />در حال بارگذاری امتیازهای شما…</div> : error ? <div className="space-y-3"><ErrorAlert>{error}</ErrorAlert><Button type="button" variant="outline" onClick={loadClub}>تلاش دوباره</Button></div> : (
          <>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              ...
              ].map(([label, value, Icon, accent]) => <div key={label} className="rounded-2xl border border-border/50 bg-background/60 p-4 shadow-sm"><Icon className={`mb-3 h-5 w-5 ${accent}`} /><p className="text-2xl font-black tabular-nums text-foreground">{Number(value || 0).toLocaleString('fa-IR')}</p><p className="mt-1 text-xs font-semibold text-muted-foreground">{label}</p></div>)}
            </div>
            ...
            <div>
              ...
              {history.length === 0 ? <div className="rounded-2xl border border-dashed border-border/60 p-6 text-center text-sm text-muted-foreground">تاریخچه دریافت پاداش‌های شما در اینجا نمایش داده می‌شود.</div> : <div className="max-h-72 space-y-2 overflow-y-auto pr-1">{history.map((entry) => <div key={entry.id} className="flex items-center gap-3 rounded-2xl border border-border/50 bg-background/55 p-3">{entry.status === 'available' ? <CheckCircle className="h-5 w-5 shrink-0 text-emerald-500" /> : <XCircle className="h-5 w-5 shrink-0 text-muted-foreground" />}<div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-foreground">{entry.rule_name || 'پاداش امتیازی'}</p><p className="mt-0.5 text-xs text-muted-foreground">{dateTime(entry.redeemed_at)} · {entry.status}</p></div><span className="shrink-0 text-sm font-black text-violet-600 dark:text-violet-400">−{Number(entry.points_cost || 0).toLocaleString('fa-IR')}</span></div>)}</div>}
            </div>
