...
const ProductCard = ({
  ...
  const prefetchProduct = useCallback(() => {
    ...
    queryClient.prefetchQuery(productDetailQueryOptions(product.slug));
  ...
  }, [buildTransform, isScene, schedulePrefetch]);
  ...
  const handleMouseLeave = useCallback(() => {
    ...
    cardRef.current.style.transition = `transform ${TRANSITION_MS}ms cubic-bezier(.23,1,.32,1), box-shadow ${TRANSITION_MS}ms cubic-bezier(.23,1,.32,1)`;
  }, [buildTransform, cancelScheduledPrefetch, isScene]);
    ...
    <>
      ...
      >
        <ResponsiveImage
          ...
          sizes="(min-width: 1280px) 260px, (min-width: 1024px) 22vw, (min-width: 640px) 42vw, 78vw"
          width="600"
          height="800"
  ...
  const wrapperProps = onNavigate
    ? {
        ...
        onPointerDown: (event) => {
          if (event.pointerType === 'touch') prefetchProduct();
        },
        onKeyDown: (e) => {
  ...
  return (
    <Wrapper {...wrapperProps}>
      <div
        ...
        onMouseEnter={isScene ? () => { setIsHovered(true); schedulePrefetch(); } : handleMouseEnter}
        onMouseLeave={isScene ? () => { setIsHovered(false); cancelScheduledPrefetch(); } : handleMouseLeave}
      >
        <div
